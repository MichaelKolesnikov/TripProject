...
\lstset{
    ...
    morekeywords={CREATE, TABLE, SELECT, INSERT, UPDATE, DELETE, WHERE, FROM, INTO, VALUES, SET, VARCHAR, INT, PRIMARY, KEY, NOT, NULL, AUTO_INCREMENT},
...
\begin{enumerate}
    ...
    \item Функциональная модель
          ...
          Менеджер может посмотреть информацию о поездке у любого пользователя, изменить информацию о поездке у любого пользователя,
          если не сделано изменение, посланное клиентом, создать запись о поездке для клиента.

    \item Алгоритмическая модель

          \includegraphics[width=\textwidth]{media/models/alg_model.jpg}

          Здесь представлен алгоритм для редактирования информации о поездке менеджером. Ключевым моментом является то,
          что блокируется данная заявка на изменение в базе данных, проверяется, что данная заявка никем не обрабатывается,
          после чего в данную заявку на изменение помещается id менеджера, чтобы никакой другой менеджер уже не смог работать над данной заявкой.
          Только при успешном прохождении данных этапов менеджер сможет работать над данной заявкой на изменение.
...
\begin{enumerate}
    ...
    \item При долгом бездействии сессиия клиента завершается. При перезагрузке страницы или при попытке отправить заявку на изменение (она не будет отправлена),
          пользователя перенаправит на страницу авторизации и выведется сообщение: \\
          \includegraphics[scale=0.3]{media/client/end_session.png} \\
    \item Если пользователь выйдет или у сессия завершится из-за бездействия, то в cookie останется на 5 минут его login,
          который будет автоматически подставляться в поле login
\end{enumerate}
